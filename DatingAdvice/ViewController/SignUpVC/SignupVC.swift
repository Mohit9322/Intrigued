...
var firstTimeCoachSignUp:String = "NO"
   var PricingDictManage = ["DirectPricing" :"$0.00" , "RushDirectPricing" : "$0.00", "LiveChatPricing" :"$0.00"]
class SignupVC: UIViewController,UIImagePickerControllerDelegate,UINavigationControllerDelegate,LocationManagerDelegate,UITextViewDelegate,UITextFieldDelegate,UIPickerViewDelegate,UIPickerViewDataSource,UICollectionViewDataSource, UICollectionViewDelegateFlowLayout, UICollectionViewDelegate {

    @IBOutlet weak var nameUserProfileLbl: UILabel!
    ...
    var direct       = NSMutableDictionary()
    var rushDirect =  NSMutableDictionary()
    var LiveChat =  NSMutableDictionary()
...
//        direct = ["State":"UnSelected", "PricingValue":"$0.00"]
//        rushDirect = ["State":"UnSelected", "PricingValue":"$0.00"]
//        liveChat = ["State":"UnSelected", "PricingValue":"$0.00"]
      
       
        self.BasicPricingArray = getPricingList() as! NSMutableArray
        print(self.BasicPricingArray)
        
        self.tf_emailId.isUserInteractionEnabled = false
        self.signUpTableView.isHidden = true
        ...
        self.signUpTableView.contentInset = UIEdgeInsetsMake(-38, 0, 0, 0);
        showInitialDetailsOnView()
        registerCellNib()
        EditUserBtn.addTarget(self, action: #selector(changeProfileImageAcion), for: .touchUpInside)
...
//
      ...
      //  let interval = 1.0
        ...
        self.signUpTableView.register(UITableViewCell.self, forCellReuseIdentifier: "CategoryCell")
        ...
        if UserOrCoachSignUpPopup == 0 {
            self.segmentControl.selectedSegmentIndex = 0
            UserOrCoachSignUpPopup = 2
            self.signUpTableView.isHidden = true
            signupView.isHidden = false
        }else if UserOrCoachSignUpPopup == 1 {
    ...
    @objc func DoneBtnPressed(_ sender: UIButton) {
        ...
        var ServiceTaxPercent =  serviceTaxPercent
     ...
     //    intrigued has a 25% fee.  Your actual rate will be $2.42”
        ...
        alertView.addAction(UIAlertAction(title: "Cancel", style: .default, handler: { action in
            ...
            self.signUpTableView.reloadRows(at: [IndexPath(row: sender.tag, section: 3)], with: .none)
        }))
        
        present(alertView, animated: true, completion: nil)
    }
    func showInitialDetailsOnView() {
        print("social info === ",socialInfo)
        emailID = socialInfo["email"] as? String ?? ""
        tf_emailId.text = emailID
        firstName = socialInfo["firstname"] as? String ?? ""
        tf_FirstName.text = firstName
        lastName = socialInfo["lastname"] as? String ?? ""
        tf_LastName.text = lastName
        socialId = socialInfo["socialId"] as? String ?? ""
        let image_Url = socialInfo["profilepic"] as? String
        ...
        if image_Url == "" {
            ...
            nameUserProfileLbl.isHidden = false
             userProfileImage.isHidden = true
        
            nameUserProfileLbl.text = String(firstName.prefix(1))
            nameUserProfileLbl.textColor = UIColor.white
            nameUserProfileLbl.backgroundColor = hexStringToUIColor(hex: "#1d7399")
            nameUserProfileLbl.textAlignment = .center
            nameUserProfileLbl.font = UIFont.boldSystemFont(ofSize: 50)
            nameUserProfileLbl.layer.masksToBounds = true
            nameUserProfileLbl.layer.cornerRadius = 50.0
           
        }else {
            nameUserProfileLbl.isHidden = true
            userProfileImage.isHidden = false
        }
        
        user_address = APPDELEGATE.currentAddress
        tf_address.text = user_address
        locationArray = [APPDELEGATE.currentLongitude,APPDELEGATE.currectLatitude]
        print(locationArray)
...
//            }
//        }
//        self.signUpTableView.reloadRows(at: [IndexPath(row: 0, section: 2)], with: .none)
//
//
//    }
        ...
        if text == "$0.00" {
            ...
            if tag == 0 {
                ...
                if status1 == "YES" {
                    ...
                    let indexPath = IndexPath(row:activeField.tag , section: 3)
                    let cell = self.signUpTableView.cellForRow(at: indexPath) as! PricingSignUpTblCell
    ...
    func managePrice(tagValue : Int) {
        ...
        if selectedIndex == 0 {
            ...
            if status1 == "YES" {
                PriceModel1?.coachPriceStatus = "NO"
                PriceModel1?.coachPrice = "$0.00"
                self.signUpTableView.reloadRows(at: [IndexPath(row: selectedIndex, section: 3)], with: .none)
                ...
                DispatchQueue.main.asyncAfter(deadline: .now() + 0.1) {
                    let indexPath = IndexPath(row:self.selectedIndex , section: 3)
                    let cell = self.signUpTableView.cellForRow(at: indexPath) as! PricingSignUpTblCell
...
//        catHighImageArray = ["tarot_reading_select","psychic_reading_sel","relationship_reading_select"]
//
//        var catModel1 =  CategoryModel.init() as CategoryModel
//        catModel1.categoryName = "Single"
//        catModel1.CatSelectedImgName = "tarot_reading_select"
//        catModel1.CatUnSelectedImgName = "tarot_reading_unselect"
//        catModel1.CatSelectedStatus = "NO"
//        print(catModel1)
//
//        var catModel2 =  CategoryModel.init() as CategoryModel
//        catModel2.categoryName = "New Relationship"
//        catModel2.CatSelectedImgName = "psychic_reading_sel"
//        catModel2.CatUnSelectedImgName = "psychic_reading_unsel"
//        catModel2.CatSelectedStatus = "NO"
//        print(catModel2)
//
//        var catModel3 =  CategoryModel.init() as CategoryModel
//        catModel3.categoryName = "Long Term"
//        catModel3.CatSelectedImgName = "relationship_reading_select"
//        catModel3.CatUnSelectedImgName = "relationship_reading_unselect"
//        catModel3.CatSelectedStatus = "NO"
//        print(catModel3)
//
//        print(categoryArray)
...
//        print(categoryArray)
//
//        if categoryArray.contains("Single") {
//            catModel1.CatSelectedStatus = "YES"
//        }
//        if categoryArray.contains("New Relationship") {
//            catModel2.CatSelectedStatus = "YES"
//        }
//        if categoryArray.contains("Long Term") {
//            catModel3.CatSelectedStatus = "YES"
//        }
//        categoryModelArray.insert(catModel1, at: 0)
//        categoryModelArray.insert(catModel2, at: 1)
//        categoryModelArray.insert(catModel3, at: 2)
//        print(categoryModelArray.count)
//        print(categoryModelArray)
//    }
    ...
    @IBAction func changeProfilePicAcion(_ sender: Any) {
    ...
    @objc func changeProfileImageAcion(_ sender: Any) {
        let actionSheetController = UIAlertController(title: "Choose Image", message:nil , preferredStyle: .actionSheet)
    ...
    @IBAction func sinupButtonAction(_ sender: Any) {
        if segmentControl.selectedSegmentIndex == 0 {
            if ischeckUserManadoryDetails() {
...
//            if status1 == "YES" {
//
//                categoryArray.add(CatName1)
//
//            }else if status1 == "NO" {
//
//            }
//            if status2 == "YES" {
//                categoryArray.add(CatName2)
//            }else if status2 == "NO" {
//
//            }
//            if status3 == "YES" {
//                categoryArray.add(CatName3)
//            }else if status3 == "NO" {
//
//            }
            ...
            print(categoryArray)
            print(livechat_Status)
            print(direct_Status)
            print(rush_direct_Status)
            print(liveChat)
            print(directMessage)
            print(rushDirectMessage)
    ...
    func imagePickerController(_ picker: UIImagePickerController, didFinishPickingMediaWithInfo info: [String : Any]){
        ...
        if segmentControl.selectedSegmentIndex == 0 {
           ...
           userProfileImage.image = info[UIImagePickerControllerOriginalImage] as? UIImage
            ...
            if let cell = signUpTableView.cellForRow(at: infoindexPath) as? CoachesSignUpCell {
        ...
        uploadImage = uploadImage.resizeWithWidth(width: 200)!
        let fileManager = FileManager.default
        let path = (NSSearchPathForDirectoriesInDomains(.documentDirectory, .userDomainMask, true)[0] as NSString).appendingPathComponent("\(String(describing: uploadImage)).jpeg")
    ...
    //MARK: ********** Complete Profile Api for Send Mail ****************
    func completeProfileApi(){
        
        let request = ["email":tf_emailId.text ?? ""] as [String:Any]
        
        WebServices().mainFunctiontoGetDetails(data:request as NSDictionary,serviceType:KComplete_Profile_Mail)
        { (responseData) in
            if responseData != nil
            {
    ...
    func signupUser() {
        ...
        tf_address.text = "Noida"
        let request = ["socialId":socialId, "socialType":socialType, "fname": tf_FirstName.text ?? "", "lname":tf_LastName.text ?? "", "email":tf_emailId.text ?? "", "address":tf_address.text ?? "noida" ,"longlat":locationArray, "phone_no":tf_phone.text ?? "" , "profile_pic":uploadUrl,"type":1, "user_devices":[userDeviceArray]] as [String : Any]
        ...
        WebServices().mainFunctiontoGetDetails(data: request as NSDictionary,serviceType:kLOGIN_USER) { (responseData)  in
            ...
            if responseData != nil
            {
                ...
                if code == 200{
                  ...
                  showAlerView(title: "Alert", message: "Welcome to Intrigued!", self1: self)
                }
                else {
    ...
    func signupAs_Coach() {
        ...
        let request = ["socialId":socialId, "socialType":socialType, "fname": firstName, "lname":lastName, "email":tf_emailId.text ?? "", "address":user_address ,"longlat":locationArray, "phone_no":phoneNumber, "profile_pic":uploadUrl ,"about_services": aboutcategory,"about":aboutyou,"categories":categoryArray,"direct_price":directMessage,
            "rush_direct_price":rushDirectMessage,"livechat_price":liveChat,"type": 2,
            "direct_Status":direct_Status,
           "rush_direct_Status":rush_direct_Status,
           "livechat_Status":livechat_Status, "user_devices":[userDeviceArray]] as [String : Any]
        ...
        WebServices().mainFunctiontoGetDetails(data: request as NSDictionary,serviceType:kLOGIN_USER) { (responseData)  in
            ...
            if responseData != nil
            {
                ...
                //    pushView(viewController: self, identifier: "CoachesTabbarVC")
                    ...
                    let titleAttrString = NSMutableAttributedString(string: "Make yourself known!", attributes: titleFont)
                    let messageAttrString = NSMutableAttributedString(string: "\n Would you like to record introduction video?", attributes: messageFont)
...
//                            nextViewController.coachDetails = resultDict as NSDictionary as! [AnyHashable : Any]
//                            nextViewController.fromCoachSignUp = "YES"; self.navigationController?.pushViewController(nextViewController, animated: true)
                            
                            
                            let loginVC = UIStoryboard(name: "Main", bundle: nil).instantiateViewController(withIdentifier: "VideoCameraVc") as! VideoCameraVc
                            loginVC.coachDetails = resultDict as NSDictionary
                            loginVC.fromCoachSignUp = "YES"
...
//                    pushView(viewController: self, identifier: "VideoCameraVc")
    ...
    func ischeckUserManadoryDetails() -> Bool {
        
        if String.isNilOrEmpty(self.tf_FirstName.text)  {
            notifyUser("", message: kFIRSTNMAE_BLANK, vc: self)
            return false
        }
        else if String.isNilOrEmpty(self.tf_LastName.text)  {
            notifyUser("", message: kLASTNMAE_BLANK, vc: self)
            return false
        }
        else if String.isNilOrEmpty(self.tf_emailId.text)  {
...
//        else if String.isNilOrEmpty(self.tf_phone.text) {
//            notifyUser("", message: kPHONE_BLANK, vc: self)
//            return false
//        }
//        else if String.isNilOrEmpty(self.tf_address.text) {
...
//        }
    ...
    func ischeckCoachesManadoryDetails() -> Bool {
        if let cell = signUpTableView.cellForRow(at: IndexPath(row: 0, section: 2)) as? CoachesCategoryCell {
            
              categoryArray =  cell.coachesCategoryArray
            print(cell.coachesCategoryArray)
            print(cell.coachesCategoryArray.count)
            print(categoryArray.count)
            print(categoryArray)
        }
        if let cell = signUpTableView.cellForRow(at: IndexPath(row: 0, section: 3)) as? CoachesPricingCell{
            // do what you need with cell
            directMessage = cell.tf_Price.text ?? ""
        }
        if let cell = signUpTableView.cellForRow(at: IndexPath(row: 1, section: 3)) as? CoachesPricingCell{
            // do what you need with cell
            rushDirectMessage = cell.tf_Price.text ?? ""
        }
        if let cell = signUpTableView.cellForRow(at: IndexPath(row: 2, section: 3)) as? CoachesPricingCell{
            // do what you need with cell
        ...
        else if String.isNilOrEmpty(emailID)  {
            notifyUser("", message: kEMAIL_BLANK, vc: self)
            return false
        }
        else if !String.isValidEmail(emailID) {
            notifyUser("", message:kVALID_EMAIL, vc: self)
            return false
        }
...
//        if aboutcategory == "Write here 500 character" {
   ...
   //MARK:LocationManagerDelegate
    func getCurrentLocation(address: String, latitude: String, longitude: String) {
        ...
        if segmentControl.selectedSegmentIndex == 0 {
            ...
            if let cell = signUpTableView.cellForRow(at: addressindexPath) as? AddUserAddressCell {
                // do what you need with cell
    ...
    func registerCellNib(){
        ...
        signUpTableView.register(UINib(nibName: "CoachesSignUpCell", bundle: nil), forCellReuseIdentifier: "CoachesSignUpCell")
        signUpTableView.register(UINib(nibName: "CoachesCategoryCell", bundle: nil), forCellReuseIdentifier: "CoachesCategoryCell")
        signUpTableView.register(UINib(nibName: "CoachesPricingCell", bundle: nil), forCellReuseIdentifier: "CoachesPricingCell")
        signUpTableView.register(UINib(nibName: "CoachesAddAboutCell", bundle: nil), forCellReuseIdentifier: "CoachesAddAboutCell")
        signUpTableView.register(UINib(nibName: "AddUserAddressCell", bundle: nil), forCellReuseIdentifier: "AddUserAddressCell")
         signUpTableView.register(UINib(nibName: "PricingSignUpTblCell", bundle: nil), forCellReuseIdentifier: "PricingSignUpTblCell")
        categoryCellName = "Category"
        self.signUpTableView.register(UITableViewCell.self, forCellReuseIdentifier: categoryCellName)
        self.signUpTableView.register(UINib(nibName: "PricingSignUpTblCell", bundle: nil), forCellReuseIdentifier: "PricingSignUpTblCell")
    }
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
         if indexPath.section == 0 {
            if indexPath.row == 0 {
                ...
                cell.EditBtn.addTarget(self, action: #selector(changeProfileImageAcion), for: .touchUpInside)
                ...
                cell.Tf_phoneNo.keyboardType = .numberPad
...
//             cell.selectionStyle = .none
//             cell.setupData(isEdit:true)
//             return cell
            
            
//            /****************  New 1 *************/
//            let cell:UITableViewCell = self.signUpTableView.dequeueReusableCell(withIdentifier: categoryCellName) as UITableViewCell!
//
//            cell.frame =  CGRect(x: 0, y: 0, width: self.view.frame.size.width , height: 100)
...
//            return cell
            ...
            cell.PriceTxtFieldBaseView.frame = CGRect(x:cell.ChatTypeBtn.frame.size.width + cell.ChatTypeBtn.frame.origin.x + 10.5, y: 0, width: 80, height: cell.BaseView.frame.size.height)
